...
/*
TimerTime_t TxTimeoutTimer;
TimerTime_t RxTimeoutTimer;
TimerTime_t RxTimeoutSyncWord;
*/
/*
...
void SX1276Reset( void )
{
    ...
    HAL_GPIO_WritePin(RADIO_RESET_GPIO_Port, RADIO_RESET_Pin, 1);
...
void SX1276ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    uint8_t dummy;
    HAL_SPI_TransmitReceive(&hspi1, &txData, &dummy, 1, 1000);

    for( i = 0; i < size; i++ )
    {
        uint8_t txData = 0;
        HAL_SPI_TransmitReceive(&hspi1, &txData, &buffer[i], 1, 1000);
    }
